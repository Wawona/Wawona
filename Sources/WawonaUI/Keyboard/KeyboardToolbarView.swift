...
open class KeyboardToolbarView: UIView {
    private nonisolated static let logger = Logger(subsystem: "io.wawona.Wawona", category: "KeyboardToolbar")
    ...
    public var onDrawerStateChanged: (() -> Void)?
    ...
    public private(set) var sizes: KeyboardSizes
    ...
    public private(set) var drawerState: DrawerState = .closed
    ...
    private func setupViews() {
        ...
        let backgroundLeading = backgroundBlurView.leadingAnchor.constraint(equalTo: leadingAnchor)
        let backgroundTrailing = backgroundBlurView.trailingAnchor.constraint(equalTo: trailingAnchor)
        backgroundLeadingConstraint = backgroundLeading
        backgroundTrailingConstraint = backgroundTrailing
        let topBorderLeading = topBorder.leadingAnchor.constraint(equalTo: leadingAnchor)
        let topBorderTrailing = topBorder.trailingAnchor.constraint(equalTo: trailingAnchor)
        topBorderLeadingConstraint = topBorderLeading
        topBorderTrailingConstraint = topBorderTrailing
        let drawerContainerLeading = drawerContainerView.leadingAnchor.constraint(equalTo: leadingAnchor)
        let drawerContainerTrailing = drawerContainerView.trailingAnchor.constraint(equalTo: trailingAnchor)
        drawerContainerLeadingConstraint = drawerContainerLeading
        drawerContainerTrailingConstraint = drawerContainerTrailing
        let toolbarHeight = mainToolbarContainer.heightAnchor.constraint(equalToConstant: sizes.toolbar.height)
        mainToolbarHeightConstraint = toolbarHeight
        let mainToolbarLeading = mainToolbarContainer.leadingAnchor.constraint(equalTo: leadingAnchor)
        let mainToolbarTrailing = mainToolbarContainer.trailingAnchor.constraint(equalTo: trailingAnchor)
        mainToolbarLeadingConstraint = mainToolbarLeading
        mainToolbarTrailingConstraint = mainToolbarTrailing
        ...
        let constraints: [NSLayoutConstraint] = [
            ...
            drawerContainerLeading,
            drawerContainerTrailing,
            heightConstraint,
    ...
    public func setBottomEdgeSquared(_ squared: Bool) {
    ...
    public func rebuildForCurrentWidth() {
        let width = bounds.width
        guard width > 0 else { return }
    ...
    private func updateModifierState(_ modifier: KeyModifiers?, state: ModifierState) {
        ...
        Self.logger.debug("KeyboardToolbar: Updated modifiers to rawValue: \(rawValue)")
        onModifiersChanged?(activeModifiers)
    }

    public func clearOneShotModifiers() {
        var didChange = false
        ...
        if didChange {
            let rawValue = activeModifiers.rawValue
            Self.logger.debug("KeyboardToolbar: Cleared one-shot modifiers, remaining rawValue: \(rawValue)")
            onModifiersChanged?(activeModifiers)
        }
    }

    public func clearModifiers() {
        activeModifiers = []
        modifierStates.removeAll()
        for button in modifierButtons {
            button.reset()
        }
        Self.logger.debug("KeyboardToolbar: Cleared all modifiers")
        onModifiersChanged?(activeModifiers)
    }
    ...
    public func setDefersKeysForBottomEdgeGesture(_ defers: Bool) {
    ...
    public func updateSizes(_ newSizes: KeyboardSizes) {
        sizes = newSizes
        layer.cornerRadius = sizes.toolbar.cornerRadius
        layer.cornerCurve = .continuous
        backgroundBlurView.layer.cornerRadius = sizes.toolbar.cornerRadius
    ...
    public var plateLeadingAnchor: NSLayoutXAxisAnchor { backgroundBlurView.leadingAnchor }
    public var plateTrailingAnchor: NSLayoutXAxisAnchor { backgroundBlurView.trailingAnchor }

    public func glassTintColor(for traitCollection: UITraitCollection) -> UIColor {
...
extension KeyboardToolbarView: KeyboardButtonDelegate {
    public func keyPressed(_ key: String, modifiers: KeyModifiers) {
        ...
        let combinedModifiers = combinedModifiersIncludingSystemShift(modifiers)
        Self.logger.debug("KeyboardToolbar: key=\(key), activeModifiers=\(self.activeModifiers.rawValue), combined=\(combinedModifiers.rawValue)")
