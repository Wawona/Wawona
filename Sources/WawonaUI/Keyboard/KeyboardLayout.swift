...
//  Wawona
//
//  Defines keyboard toolbar layouts for different devices (ported 1:1 from rootshell).
//
...
public enum KeyDefinition: Sendable {
    // Modifiers
    case esc
    case ctrl
    case alt
...
public struct KeyboardLayout: Sendable {
    public let leftSection: [KeyDefinition]
    public let middleSection: [KeyDefinition]
    public let rightSection: [KeyDefinition]

    // MARK: - Device-Specific Layouts

    public static func layout(for device: UIUserInterfaceIdiom) -> KeyboardLayout {
        switch device {
        case .pad:
            return iPadLayout()
        case .phone:
