...
fn test_window_maximized_transition() {
    ...
    assert_eq!(
        client_state.last_width, 0,
        "Unset maximized should yield floating configure width"
    );
    assert_eq!(
        client_state.last_height, 0,
        "Unset maximized should yield floating configure height"
    );
    assert!(
        !client_state.maximized,
...
fn test_configure_serial_backlog_without_ack() {
    ...
    assert!(
        first_serial > 0,
        "initial configure should produce a serial"
    );
