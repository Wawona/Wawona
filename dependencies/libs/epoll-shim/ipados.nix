...
pkgs.stdenv.mkDerivation {
  ...
  buildInputs = [ ];
  postPatch = ''
    # Disable tests for iPadOS cross-compilation using Nix patches
    # Tests can't run during cross-compilation and cause build failures
    if [ -f CMakeLists.txt ]; then
      # Disable enable_testing() call
      substituteInPlace CMakeLists.txt \
        --replace "enable_testing()" "# enable_testing() # Disabled for iPadOS cross-compilation" || true
      
      # Disable test subdirectory
      substituteInPlace CMakeLists.txt \
        --replace "add_subdirectory(test)" "# add_subdirectory(test) # Disabled for iPadOS cross-compilation" || true
      ...
      substituteInPlace CMakeLists.txt \
        --replace "include(CTest)" "# include(CTest) # Disabled for iPadOS cross-compilation" || true

      # Upstream occasionally reformats these lines; regex-based edits keep
      # tests disabled even when spacing/indentation changes.
      perl -0pi -e 's/^\s*enable_testing\(\)\s*$/# enable_testing() # Disabled for iPadOS cross-compilation/mg' CMakeLists.txt
      perl -0pi -e 's/^\s*include\(\s*CTest\s*\)\s*$/# include(CTest) # Disabled for iPadOS cross-compilation/mg' CMakeLists.txt
      perl -0pi -e 's/^\s*add_subdirectory\(\s*test\s*\)\s*$/# add_subdirectory(test) # Disabled for iPadOS cross-compilation/mg' CMakeLists.txt
      perl -0pi -e 's/^\s*if\s*\(\s*BUILD_TESTING\s*\)\s*$/if(FALSE AND BUILD_TESTING)/mg' CMakeLists.txt
      if [ -f src/CMakeLists.txt ]; then
        perl -0pi -e 's/^\s*add_subdirectory\(\s*external\/microatf\s*\)\s*$/# add_subdirectory(external\/microatf) # Disabled for iPadOS cross-compilation/mg' src/CMakeLists.txt
      fi

      # CMake 4 thread probing fails in iPadOS cross CI even though pthreads are
      # available through the SDK. Provide a synthetic Threads::Threads target
      # on iPadOS while retaining the normal find_package path elsewhere.
      if [ -f src/CMakeLists.txt ]; then
        perl -0pi -e 's/find_package\\(Threads REQUIRED\\)/if\\(IOS\\)\\n  if\\(NOT TARGET Threads::Threads\\)\\n    add_library\\(Threads::Threads INTERFACE IMPORTED\\)\\n  endif\\(\\)\\nelse\\(\\)\\n  find_package\\(Threads REQUIRED\\)\\nendif\\(\\)/g' src/CMakeLists.txt
      fi
      
      # Fix sysctl warning on iPadOS (discard qualifiers)
      ...
      echo "Patched CMakeLists.txt and sources for iPadOS cross-compilation"
    fi
  '';
  preConfigure = ''
    ...
    # CMake thread probing fails under iPadOS cross-compile in CI; force pthreads.
  ...
  cmakeFlags = [
    ...
    "-DBUILD_TESTING=OFF"
    # Build static library for iPadOS (shared libraries require code signing)
  ...
  postInstall = ''
        ...
        echo "=== Verifying epoll-shim iPadOS build ==="
        ...
        if [ -f "$out/lib/libepoll-shim.a" ]; then
          ...
          # Verify library architecture (should be arm64 for iPadOS)
          if command -v file >/dev/null 2>&1; then
            ...
            if file "$out/lib/libepoll-shim.a" | grep -q "arm64\|ARM64\|aarch64"; then
              echo "✓ Library architecture is correct for iPadOS (arm64)"
            else
              echo "⚠ Warning: Library architecture may not be correct for iPadOS"
              file "$out/lib/libepoll-shim.a" || true
            fi
          fi
          ...
          # Re-setup iPadOS toolchain for link test
    ...
    TESTCODE
          ...
          if [ -n "$RE_SDKROOT" ] && [ -n "$RE_IOS_CC" ] && [ -x "$RE_IOS_CC" ]; then
            echo "Running link test: Compiling test program with iPadOS toolchain..."
            ...
            TEST_OUTPUT=$("$RE_IOS_CC" -isysroot "$RE_SDKROOT" \
               ...
               $PKG_LIBS \
               test_link.c \
               -o test_link_ipados 2>&1)
            TEST_EXIT_CODE=$?
            
            if [ $TEST_EXIT_CODE -eq 0 ] && [ -f test_link_ipados ]; then
              echo "✓ Test program compiled and linked successfully"
              echo "✓ Library is linkable and headers are correct"
              
              # Verify the test binary is for iPadOS arm64
              if command -v file >/dev/null 2>&1; then
                BINARY_ARCH=$(file test_link_ipados | grep -o "arm64\|ARM64\|aarch64" || echo "")
                if [ -n "$BINARY_ARCH" ]; then
                  echo "✓ Test binary architecture is correct: $BINARY_ARCH"
                fi
              fi
              
              rm -f test_link.c test_link_ipados 2>/dev/null || true
            else
            ...
            echo "✗ ERROR: Cannot run link test - iPadOS toolchain not available"
          ...
          echo "=== epoll-shim iPadOS build verification complete ==="
          echo "✓ Library built successfully for iPadOS"
        else
