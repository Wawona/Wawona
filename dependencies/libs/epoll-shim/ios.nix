...
pkgs.stdenv.mkDerivation {
  ...
  preConfigure = ''
    ...
    set(CMAKE_C_FLAGS "-target $APPLE_LINKER_TARGET -isysroot $SDKROOT")
    set(CMAKE_CXX_FLAGS "-target $APPLE_LINKER_TARGET -isysroot $SDKROOT")
  ...
  configurePhase = ''
    ...
    if [ -n "$SDKROOT_VAL" ]; then
      EXTRA_CMAKE_FLAGS="-DCMAKE_OSX_SYSROOT=$SDKROOT_VAL -DCMAKE_OSX_DEPLOYMENT_TARGET=${mobileMinVersion}"
    fi
    cmake -B build -S . \
      -DCMAKE_TOOLCHAIN_FILE=ios-toolchain.cmake \
      -DCMAKE_BUILD_TYPE=Release \
  ...
  '';
  postInstall = ''
        if [ "${if iosToolchain.isVisionOSToolchain or false || iosToolchain.isTVOSToolchain or false then "1" else "0"}" = "1" ]; then
          echo "Skipping iOS-specific link verification for visionOS/tvOS build"
          exit 0
        fi
