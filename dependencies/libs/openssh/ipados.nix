...
let
  ...
  zlib = buildModule.buildForIPadOS "zlib" { inherit simulator; };
  # OpenSSL - we need to build it for iPadOS
  openssl = let
...
pkgs.stdenv.mkDerivation {
  ...
  postPatch = ''
    # ========================================
    # iPadOS Compatibility Patches for OpenSSH
    # ========================================
    
    echo "========================================"
    echo "Applying comprehensive iPadOS patches..."
    echo "========================================"
    
    # Fix getrrsetbyname.c for iPadOS - DNS resolver types not available
...
EOF
    ...
    # Fix clientloop.c - system() is unavailable on iPadOS
    if [ -f clientloop.c ]; then
      sed -i.bak 's/if (system(cmd) == 0)/if (0 \/* system() unavailable on iPadOS *\/)/' clientloop.c
      rm -f clientloop.c.bak
    fi
    
    # ========================================
    # CRITICAL: Patch readpassphrase to use SSH_ASKPASS_PASSWORD env var on iPadOS
    # iPadOS doesn't have a TTY, so we must get password from environment
    # ========================================
    echo "Patching readpassphrase.c for iPadOS environment-based password..."
...
readpassphrase(const char *prompt, char *buf, size_t bufsiz, int flags)
{
    ...
    /* 
     * iPadOS-specific: Check for password in environment variable first
     * This is the primary method on iPadOS since we don't have a TTY
     */
    ...
    /* 
     * iPadOS fallback: If no environment password and no TTY,
...
READPASS_EOF
      ...
      echo "✓ Patched readpassphrase.c for iPadOS password handling"
    fi
    ...
    # ========================================
    echo "Patching readpass.c for iPadOS environment-based password..."
...
\	}
      ...
      rm -f readpass.c.bak
      echo "✓ Patched readpass.c for iPadOS password handling"
    fi
    
    # ========================================
    # Patch sshconnect.c to handle iPadOS-specific connection issues
    # ========================================
    echo "Patching sshconnect.c for iPadOS..."
    ...
    # ========================================
    # Patch misc.c for iPadOS tty handling
    # ========================================
    echo "Patching misc.c for iPadOS tty handling..."
    if [ -f misc.c ]; then
      # Replace ttyname calls that fail on iPadOS
    ...
    # ========================================
    # Critical: Fix getpwuid() failures on iPadOS
    # iPadOS Simulator doesn't have /etc/passwd
    # ========================================
    
    # Patch ssh.c to provide fallback user when getpwuid fails
    if [ -f ssh.c ]; then
      echo "Patching ssh.c for iPadOS user lookup..."
      python3 << 'PYTHON_EOF'
import re
import sys
...
def replacement(indent):
    return (indent + "pw = getpwuid(getuid());\n" +
            indent + "if (pw == NULL) {\n" +
            indent + "\t/* iPadOS fallback: create synthetic passwd entry */\n" +
...
PYTHON_EOF
    ...
    if [ -f uidswap.c ] && grep -q 'No user exists for uid' uidswap.c; then
      echo "Patching uidswap.c for iPadOS user lookup..."
      python3 << 'PYTHON_EOF'
import re
import sys
...
def repl(m):
    i = m.group("indent")
    return (i + "/* iPadOS fallback: return synthetic passwd entry instead of NULL */\n" +
            i + "{\n" +
...
PYTHON_EOF
    ...
    if [ -f ssh-keygen.c ] && grep -q 'No user exists for uid' ssh-keygen.c; then
      echo "Patching ssh-keygen.c for iPadOS user lookup..."
      python3 << 'PYTHON_EOF'
import re
import sys
...
PYTHON_EOF
    ...
    # ========================================
    echo "Creating ssh_main.c wrapper for iPadOS dlopen support..."
    cat > ssh_main.c << 'SSH_MAIN_EOF'
/*
 * ssh_main.c - Entry point wrapper for iPadOS dlopen support
 * 
 * This wrapper allows OpenSSH's ssh to be loaded as a dynamic library
 * and called via dlopen/dlsym on iPadOS.
 *
 * Key iPadOS-specific handling:
...
/* 
 * iPadOS fallback passwd structure
 * Used when getpwuid/getpwnam fail (no /etc/passwd on iPadOS)
 */
...
/*
 * Override getpwuid to provide fallback on iPadOS
...
struct passwd *getpwuid(uid_t uid) {
    ...
    if (!pw) {
        fprintf(stderr, "[ssh_main] getpwuid(%d) failed, using iPadOS fallback\n", (int)uid);
        ios_setup_passwd();
...
/*
 * Override getpwnam to provide fallback on iPadOS
 */
...
struct passwd *getpwnam(const char *name) {
    ...
    if (!pw) {
        fprintf(stderr, "[ssh_main] getpwnam(%s) failed, using iPadOS fallback\n", name ? name : "(null)");
        ios_setup_passwd();
        if (name) {
 ...
 * It sets up the iPadOS environment and calls SSH's real main().
 */
...
int ssh_main(int argc, char **argv) {
    ...
    /* 
     * Set up signal handling for iPadOS
...
SSH_MAIN_EOF
  ...
  buildPhase = ''
    ...
    echo "=========================================="
    echo "Building OpenSSH for iPadOS as dylib"
    echo "=========================================="
    ...
    # ========================================
    # STEP 3: Create ssh.dylib for iPadOS dlopen
    # ========================================
  ...
  installPhase = ''
    ...
    # Install ssh.dylib (primary for iPadOS dlopen)
    if [ -f ssh.dylib ]; then
