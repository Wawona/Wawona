...
let
  ...
  getDeps =
    ...
    map (
      ...
      if depName == "expat" then
        buildModule.buildForIPadOS "expat" { inherit simulator; }
      else if depName == "libffi" then
        buildModule.buildForIPadOS "libffi" { inherit simulator; }
  ...
  # epoll-shim: Required for iPadOS Wayland builds (implements epoll on top of kqueue)
  epollShim = buildModule.buildForIPadOS "epoll-shim" { inherit simulator; };

  # Build wayland-scanner for the build architecture (host)
  waylandScanner = buildPackages.stdenv.mkDerivation {
    name = "wayland-scanner-host-ipados";
    inherit src;
...
pkgs.stdenv.mkDerivation {
  ...
  postPatch = ''
        # iPadOS syscall compatibility
        echo "=== Applying iPadOS syscall compatibility patches ==="
        
        # Fix missing socket defines on iPadOS/Darwin (CMSG_LEN, MSG_NOSIGNAL, MSG_DONTWAIT)
        sed -i '1i\
    #ifndef MSG_NOSIGNAL\
    ...
    ' src/event-loop.c
        ...
        echo "Applied iPadOS syscall compatibility patches"
  '';
  ...
  configurePhase = ''
    ...
    unset SDKROOT
    echo "Configured epoll-shim paths for iPadOS: $EPOL_SHIM_PATH"
    meson setup build \
      --prefix=$out \
      --libdir=$out/lib \
      --cross-file=ipados-cross-file.txt \
...
Name: Wayland Client
Description: Wayland client side library (iPadOS cross-compiled)
Version: 1.23.0
Cflags: -I\''${includedir}
...
Name: Wayland Server
Description: Wayland server side library (iPadOS cross-compiled)
Version: 1.23.0
Cflags: -I\''${includedir}
...
Name: Wayland Cursor
Description: Wayland cursor library (iPadOS cross-compiled)
Version: 1.23.0
Cflags: -I\''${includedir}
