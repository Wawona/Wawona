...
let
  ...
  # SSH handshake). OpenSSL uses SecRandomCopyBytes on iPadOS which works correctly.
...
EOF
  ...
  cmakeFlags = [
    ...
    "-DBUILD_EXAMPLES=OFF"
    "-DBUILD_TESTING=OFF"
    "-DOPENSSL_ROOT_DIR=${openssl-ipados}"
    "-DOPENSSL_CRYPTO_LIBRARY=${openssl-ipados}/lib/libcrypto.a"
    "-DOPENSSL_SSL_LIBRARY=${openssl-ipados}/lib/libssl.a"
    "-DOPENSSL_INCLUDE_DIR=${openssl-ipados}/include"
  ];
}
