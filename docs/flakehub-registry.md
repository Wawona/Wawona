...
on `x86_64-darwin`; flakes whose own `nixpkgs` lock is that new must not
expose Intel Darwin `packages`/`formatter`/`apps` (anowaW / phoon-rs). Wawona
still builds Intel Mac via the aggregator’s pkgs + registry merge.
