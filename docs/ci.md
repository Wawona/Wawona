...
**Frameworks/ content silently lost during export, and append-zip SwiftSupport unreliable on ASC (builds 93/94/95):** even after the `ITMS-90433` re-sign fix, builds 93-95 kept bouncing: build 93 got `ITMS-90426` (`SwiftSupport/` missing entirely), builds 94/95 got `ITMS-90429` (10 named `libswift*.dylib` "aren't at the expected location `Frameworks`"). Build 95's own CI log proved a real, local, reproducible bug: `embed_swift_runtime_into_archive!` embedded+re-signed **15** dylibs into the archive's `Wawona.app/Frameworks`, but re-scanning `Frameworks/` in the **exported** ipa immediately afterward (before any upload) found only **10**. Cause: the dylibs were added to `Frameworks/` *after* `xcodebuild archive` had already signed `Wawona.app`, and only each individual dylib was re-signed — never the bundle itself — so the bundle's own `CodeResources` manifest never learned about the new files, and `xcodebuild -exportArchive` dropped what it couldn't reconcile. Fix: `embed_swift_runtime_into_archive!` now re-signs the **bundle itself** (`codesign --preserve-metadata=identifier,entitlements,flags` on the `.app`, not just the dylib) immediately after modifying its `Frameworks/`, watch companion first so nested code is signed before the outer app that embeds it. Separately, `inject_swift_support_from_archive!` switched from appending `SwiftSupport/` onto the exportArchive-produced ipa in place (`zip -qry`) to a full rebuild from a fully-unzipped directory (`zip -qryX` over both `Payload/` and `SwiftSupport/`, replacing the ipa) — appending with the system `zip` onto a zip produced by a different tool round-tripped fine through our own lenient `unzip` checks but was inconsistent through ASC's stricter one. A prior full-rebuild attempt had produced "altool OK, then silent ASC discard", but that predated the `CFBundleSupportedPlatforms` fix below, a far likelier explanation for a silent, error-free rejection than the rebuild mechanism. New defense-in-depth: `assert_swift_support_frameworks_parity!` (called from `assert_ipa_has_swift_support!`) re-unzips the final ipa and verifies every dylib named under `SwiftSupport/<platform>/` is physically present in the matching bundle's `Frameworks/` — exactly ASC's own invariant — so a regression like build 95's fails the build locally instead of via a rejection email days later. Validate with `fastlane ios debug_export` (`WAWONA_DEBUG_SCHEME=Wawona-iOS`) or the `Ship: beta (stores)` workflow's `workflow_dispatch` (`lane: ios debug_export`, `debug_scheme: Wawona-iOS`) before trusting a real upload.
