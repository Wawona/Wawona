...
**The real Swift Support rule (settled by build 119; supersedes the "builds 89-104 root cause" below): ASC validates watch-bearing and watchless IPAs differently.** Build 119 shipped a textbook ABI-stable iOS+Watch IPA (every ref `/usr/lib/swift/*`, minos iOS 17 / watchOS 10, clean GM `DT*` metadata, classic `Watch/` embed, no `SwiftSupport/`) and was still rejected `ITMS-90426`, while the *identical-commit* watchless tvOS/visionOS IPAs were accepted (`buildUploads` API: iOS `FAILED` errors=[90426], tvOS/visionOS `COMPLETE`) — same split on every prior build. So: **watchless IPAs** are validated as modern ABI-stable (no `SwiftSupport/`, no `Frameworks/libswift*` — the accepted shape; only genuine `SWIFT_BACKDEPLOY_LIB_NAMES` shims below their native-OS floor may ever be embedded, and none apply at our targets), while **watch-bearing IPAs** get the **legacy pre-ABI-stability validator regardless of deployment target**: each platform bundle must carry `Frameworks/libswiftX.dylib` (re-signed with our identity) *and* `SwiftSupport/<plat>/libswiftX.dylib` (byte-identical Apple-signed toolchain originals), where **X = the bundle's referenced `/usr/lib/swift` names (weak included) ∩ toolchain `swift-5.0/<plat>/` contents** — an algorithm confirmed by reproducing build 95's ITMS-90429 expected-file list name-for-name from our own binaries. `embed_swift_runtime_into_archive!` now applies this legacy mode to watch-bearing archives (populating bundle `Frameworks/` and archive-root `SwiftSupport/{iphoneos,watchos}` *before* `xcodebuild -exportArchive`, so Apple's exporter packages them the way Organizer does for pre-ABI archives; `inject_swift_support_from_archive!` is the fallback if the exporter drops `SwiftSupport/` — full `zip -qryX` rebuild, never an append). Pre-upload, `assert_ipa_has_swift_support!` re-unzips the final ipa and hard-fails unless the shape is exactly right for its class: watch-bearing → `assert_legacy_swift_support!` (set-equality with X per platform, Frameworks parity, byte-identical Apple-signed SwiftSupport copies); watchless → `SwiftSupport/` absent. No ipa reaches ASC without passing this against its actual zip contents.
