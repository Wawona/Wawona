...
| **2026-08-20 ~open app** | Same panic (`watchdogd[17227] exited` SIGTRAP) after opening Wawona.app. Engage / `--restore-aqua` / uninstall always ran `wwn-iowatchdog enable` (lldb) even when Aqua and watchdogd were healthy. **Fix:** write `iowatchdog-userspace-disabled` only after a successful Take Over disable; `restore_watchdogd` calls enable only then. Skip `--restore-aqua` before engage when WindowServer is up. Clear `DesktopReplacementEnabled` until Take Over is intentional. |
| **2026-08-20 ~again** | Third SIGTRAP panic (`watchdogd[61853]`). **Fix:** remove the lldb fallback from `wwn-iowatchdog` entirely. disable/enable fail closed without `IOServiceOpen`. Live install stubbed to refuse. Take Over stays blocked until a non-lldb IOKit path exists. |
| **2026-08-20 ~Take Over alert** | Fourth SIGTRAP panic (`watchdogd[85081]`) immediately after Settings showed the blocked Take Over message. Helper still ran `install_ws_guard` before exiting the refuse path (privileged `launchctl` against WindowServer every 10s). Cursor `lldb_mcp` may also attach and SIGTRAP `watchdogd`. **Fix:** refuse engage in-process before sudo helper; blocked path must not install ws-guard; `wwn-iowatchdog` default never opens IOKit/`lsmp` (opt-in `WWN_IOWATCHDOG_ALLOW_OPEN=1`); uninstall Mode B helper/sudoers/ws-guard from the machine; keep `lldb_mcp` off `watchdogd`. |
| **2026-08-20 ~soft-inject attempt** | Soft-inject / remote `IOConnect` into live `watchdogd` evaluated for Take Over. `status` finds `IOWatchdogUserClient` via `mach_port_kobject_description`. **Wall on 25F80:** `thread_set_state` SIGKILLs the caller; `mach_port_extract_right` on the IOKit port returns `KERN_INVALID_CAPABILITY`; GOT/auth-ptr patch is protection-failure. Take Over stays blocked. Doc: `wwn-iowatchdog/docs/macos26-iowatchdog-wall.md`. |
| **2026-08-20 ~Classic Phase 1** | Boot-arg pass (`amfi_get_out_of_my_way=1`) plus GOT-only retest: some RO COW write-same succeeds, but no proven `disable` ACK / IOConnect hijack. `thread_set_state` remains panic-class (same-day reboot during probing; prior panic `watchdogd` namespace 2 / subcode 0x5). **Phase 1.4:** stop Classic on 25F80; Take Over stays blocked; Phase 2/3 not flipped. |
